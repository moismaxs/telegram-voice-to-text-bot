...
| `ADMIN_IDS` | — | твой Telegram ID через запятую (узнать: `@userinfobot`); открывает команду `/stats` |
| `DB_PATH` | авто | файл аналитики: `/data/voices.db`, если есть `/data`, иначе `voices.db` рядом |
| `TELEADS_API_KEY` | — | ключ [TeleAds](https://teleads.pro) (выдаётся после подключения бота в панели); пусто — без рекламы |

## Аналитика: откуда знать про живых юзеров
Бот пишет в SQLite (лежит в персистентном `/data`, переживает деплои) только user_id, имя и счётчики — ни войсы, ни тексты не хранятся.

Команда **`/stats`** (только для `ADMIN_IDS`, в `/setcommands` её не добавляем):
```
📊 Статистика
👥 Юзеров всего: N (новых сегодня: M)
🎙 Расшифровок: сегодня X · неделя Y · всего Z
⏱ Средняя длина: Ns · ошибок: E
```
Считаются и авторы войсов из групп, где стоит бот. Пока ботом пользуешься только ты — там будет `1`.

## Реклама (TeleAds)

1. Подключи бота в панели [teleads.pro](https://teleads.pro) → забери SDK-ключ.
2. Поставь `TELEADS_API_KEY=<ключ>`, перезапусти. В логах должно быть `TeleAds подключён`.
3. Без ключа бот работает как раньше — реклама просто выключена.

Технически: `BapMiddleware` из пакета `teleads` висит на уровне апдейтов (`dp.update.middleware`), статистику шлёт по UDP, внутренние апдейты рекламного флоу от хендлеров скрывает сам. Ручной режим (`send_advertisement` в нужных местах) включается в настройках платформы — точки вставки добавим, когда понадобится.
